...
- `get_items` accepts 1-100 distinct positive canonical item IDs and makes one unauthenticated public `/v2/items` request at runtime. It returns caller-ordered item facts—nullable name, type, rarity, level, vendor value, flags, game types, and restrictions—with explicit unavailable-resource rows and nullable source facts when a requested public item resource is absent. Its `asOf` is the request-completion observation, not a source publication or currentness guarantee. It does not use account data, caches, material categories, recipes, or Trading Post prices.
...
Then invoke a tool through Inspector. Account-backed tools need a locally configured valid GW2 key with the required scopes; do not paste it into Inspector arguments or chat. `find_items`, `get_item_prices`, and `value_items` use only local public caches; `get_items` is public and request-time. Holdings still accepts canonical IDs, so resolve names first when needed.
