...
- `get_trading_post_activity` currently supports only `CurrentBuys` and requires `account` and `tradingpost`. It returns one complete caller-selected page of pending buy orders with canonical item ID, unit price in copper, unfulfilled quantity, creation time, checked reserved coin per row, and a selected-page reserved-coin subtotal. `page` defaults to 0, `pageSize` defaults to 50, and page size is limited to 1-200. Results preserve source order but do not claim chronological or global ordering; transaction data is documented as cached for about five minutes, and `asOf` is response completion rather than a source timestamp. Pending-buy quantities are not owned items or holdings, the subtotal is not an account-wide total, and upstream transaction IDs are not returned. Current sells, delivery, and history modes are not yet available through this tool.
...
Configure the profile's reusable runtime key using the persistent-key guide, then run `.\start.ps1`. In ChatGPT web Developer Mode, create a read-only draft app using the tunnel connection, verify that exactly fourteen tools are discovered: `find_items`, `get_items`, `get_recipes`, `get_item_prices`, `value_items`, `get_account`, `get_account_holdings`, `get_trading_post_activity`, `get_character_build`, `get_character_equipment`, `get_character_inventory`, `get_characters`, `get_legendary_armory`, and `get_wallet`. Keep the launcher running while using the app.
