...
- `get_recipes` returns bounded unauthenticated public player-discovered crafting recipe facts in `ByIds`, `InputItem`, or `OutputItem` mode. `ByIds` accepts 1-100 distinct positive recipe IDs in caller order. Selector modes accept one positive item ID, sort the complete bounded selector result, and apply local `offset`/`limit` paging before one definition lookup. Results preserve missing definitions, Item versus GuildUpgrade outputs, raw source output IDs, source-order typed ingredients, independent selector/definition observation times, and explicit non-atomic completeness. Input search indexes Item ingredients only. Output search follows raw source `output_item_id`; that value can be bogus for guild recipes and must not be treated as their semantic Item output. The tool excludes account unlocks, Mystic Forge, vendors, item enrichment, caches, Trading Post data, prices, costs, and recommendations.
...
Then invoke a tool through Inspector. Account-backed tools need a locally configured valid GW2 key with the required scopes; do not paste it into Inspector arguments or chat. `find_items`, `get_item_prices`, and `value_items` use only local public caches; `get_items` and `get_recipes` are public and request-time. Holdings still accepts canonical IDs, so resolve names first when needed.
